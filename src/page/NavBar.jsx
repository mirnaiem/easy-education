...
const NavBar = () => {
 ...
 return (
  <div className="navbar w-[90%] mx-auto bg-base-100 md:fixed z-50 top-0 left-0 md:left-16">
  <div className="navbar-start">
    ...
    <Link to='/'>
      <img className="w-20 h-20 border-blue-500 border rounded-full hidden md:flex items-center justify-center" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQpSTmn83W_oZhFASeT09nGKYDQeljZZI3V4A&s" alt="Logo" />
    </Link>
  </div>
  ...
  <div className="navbar-end gap-3">
    {user ? (
      <>
        {user.photoURL ? (
          <img className="rounded-full me-2 w-10" title={user?.displayName} src={user.photoURL} alt="User" />
        ) : (
        ...
        <button onClick={handleLogOut} className="btn bg-orange-400 text-lg text-white">LogOut</button>
      </>
    ) : (
      <div>
